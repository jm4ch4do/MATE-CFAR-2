function [ p ] = pea_chi_squared( amount,g,limit,step, p_size )
% PEA_CHI_SQUARED Computes the 'p' values of the Chi-2 statistic test Pearson
% --------------------------------------------------------------------
...
% ----------   ----------------(example)------------------------------
% p = pea_chi_squared( 1000,0.2,5,0.1,100);
% -----------------------------(example)------------------------------
...
for p_1 = 1:p_size
    ...
    obs(1) = esp(1); % avoiding the problem caused by differences in the first two values
    obs(2) = esp(2);
