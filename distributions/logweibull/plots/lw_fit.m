function [  PDF, PDFe, CDFe, CDF, x, PDF_res,CDF_res,p_chi,p_ks,a,b] = lw_fit( samples )
% LW_GEN_COMPARE Compares a histogram from samples with the ideal PDF Log-Weibull
% -------------------------------------------------------------------------------------------------
...
%           a : estimation of the scale parameter Log-Weibull                             (1 value)
%           b : estimation of the shape parameter Log-Weibull                             (1 value)
%------------   -------------------------(example)-------------------------------------------------
% [ samples ] = lw_gen( 3000, 1, 3 );
% [ PDF, PDFe, CDFe, CDF, x, PDF_res,CDF_res, p_chi,p_ks,a,b] = lw_fit( samples );
% ---------------------------------------(example)-------------------------------------------------
...
x(1,1)= 1.00001; % making the first value different from 1
ideal= lw_pdf(x, a,b);

% expected values for the ideal (theoretical) Log-Weibull distribution
esp = ideal*m*step;
