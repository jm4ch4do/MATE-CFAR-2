function [  PDF, PDFe, CDFe, CDF, x, PDF_res,CDF_res,p_chi,p_ks,c,v] = k2_fit( samples )
% K2_GEN_COMPARE Compares a histogram from samples with the ideal PDF K2
% -------------------------------------------------------------------------------------------------
...
%     c_estim : estimation of the scale parameter K2                                      (1 value)
%     v_estim : estimation of the shape parameter K2                                      (1 value)
%------------   -------------------------(example)-------------------------------------------------
% [ samples ] = k2_gen( 3000, 1, 3 );
% [ PDF, PDFe, CDFe, CDF, x, PDF_res,CDF_res, p_chi,p_ks,c,v] = k2_fit( samples );
% ---------------------------------------(example)-------------------------------------------------
...
% --- estimate parameters
[ c,v ] = k2_estim_par( samples );

% sometimes 'v' gets too high or too low value
if v>40
    v=40;
    c = sqrt(v/mean(samples));
elseif v<0.01
    v = 0.1;
    c = sqrt(v/mean(samples));
end
...
% expected values for the ideal (theoretical) K2 distribution
esp = ideal*m*step;
