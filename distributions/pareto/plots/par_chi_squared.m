function [ p ] = par_chi_squared( amount,a,b,limit,step, p_size )
% PAR_CHI_SQUARED Computes the 'p' values of the Chi-2 statistic test Pareto
% --------------------------------------------------------------------
...
% ----------   ----------------(example)------------------------------
% p = par_chi_squared( 1000,2,2,5,0.1,100);
% -----------------------------(example)------------------------------
...
for p_1 = 1:p_size
    ...
    ideal= par_pdf(x1, a,b);
    
    % expected values for the ideal (theoretical) Pareto distribution
    ...
    % the first value different from 0 in the ideal histogram is a high
    % peak which is ussually different from the histogram
    % For avoiding the mistakes that this produces in chi-2 test:
    % searching for the firts non 0 and making it equal to 'esp'
    [m,n] = size(esp);
    for n_1 = 1:n
        if esp(n_1) ~= 0 
            obs(n_1) = esp(n_1);
