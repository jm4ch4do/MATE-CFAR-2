function [  PDF, PDFe, CDFe, CDF, x, PDF_res,CDF_res,p_chi,p_ks,g] = pea_fit( samples )
% PEA_GEN_COMPARE Compares a histogram from samples with the ideal PDF Pearson
% -------------------------------------------------------------------------------------------------
...
%           g : estimation of the scale parameter Pearson                                 (1 value)
%------------   -------------------------(example)-------------------------------------------------
% [ samples ] = pea_gen( 3000, 0.2 );
% [ PDF, PDFe, CDFe, CDF, x, PDF_res,CDF_res, p_chi,p_ks,g] = pea_fit( samples );
% ---------------------------------------(example)-------------------------------------------------
...
% --- estimate parameters
[ g ] = pea_estim_par( samples );

% --- find important percentiles
sorted = sort(samples);
the_90 = sorted(floor(90/100*m)); % using 90th percentile because pearson is too speaky
the_01 = sorted(ceil(01/100*m));

% --- PDF comparison
% computing observed values
step = (the_90-the_01)/100;
limit = the_90;
x = 0:step:limit;
...
PDFe(1,n) = PDFe(1,n-1) ;%preventing the last value from being too high
...
% expected values for the ideal (theoretical) Pearson distribution
esp = ideal*m*step;
