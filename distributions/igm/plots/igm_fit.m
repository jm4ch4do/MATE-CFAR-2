function [  PDF, PDFe, CDFe, CDF, x, PDF_res,CDF_res,p_chi,p_ks,b,a] = igm_fit( samples )
% IGM_GEN_COMPARE Compares a histogram from samples with the ideal PDF InvGam
% -------------------------------------------------------------------------------------------------
...
%           b : estimation of the scale parameter InvGam                                  (1 value)
%           a : estimation of the shape parameter InvGam                                  (1 value)
%------------   -------------------------(example)-------------------------------------------------
% [ samples ] = igm_gen( 3000, 5, 5 );
% [ PDF, PDFe, CDFe, CDF, x, PDF_res,CDF_res, p_chi,p_ks,b,a] = igm_fit( samples );
% ---------------------------------------(example)-------------------------------------------------
...
% expected values for the ideal (theoretical) InvGam distribution
esp = ideal*m*step;

% expected values for the ideal (theoretical) Inverse-Gamma distribution
esp = ideal*m*step;
